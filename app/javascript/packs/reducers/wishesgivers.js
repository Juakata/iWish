...
const wishesgiversReducer = (state = [], action) => {
  ...
  switch (action.type) {
    case ADD_WISHESGIVERS:
      ...
      return clone;
    case ADD_GIVER:
      wishgivers = clone.filter(e => e.id === profile)[0].wishgivers;
      index = clone.findIndex(e => e.id === profile);
      index2 = wishgivers.findIndex(e => e.id === wish);
      clone[index].wishgivers[index2].givers.push(giver);
      return clone;
    case REMOVE_GIVER:
      wishgivers = clone.filter(e => e.id === profile)[0].wishgivers;
      index = clone.findIndex(e => e.id === profile);
      givers = wishgivers.filter(e => e.id === wish)[0].givers;
      givers = givers.filter(e => e.id !== giver);
      index2 = wishgivers.findIndex(e => e.id === wish);
